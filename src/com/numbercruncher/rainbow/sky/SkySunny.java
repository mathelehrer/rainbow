...
public class SkySunny extends Sky {
    ...
    public double getSpectralRadiance(Ray ray, double lambda) {
        ...
        double lambda0 = 550.0;
        double rayleigh = Math.pow(lambda0 / lambda, 4.0);

        // Elevation factor: more scattering near horizon
        double horizonFactor = 1.0 - 0.5 * Math.max(0, dir.z);

        // Soft forward-scattering glow around the sun (Mie-like)
        double mieGlow = 0.0;
        if (cosAngle > 0) {
            mieGlow = 0.2 * Math.pow(cosAngle, 32) * Utils.planck(lambda, sunTemperature);
        }

        double sky = skyIntensity * rayleigh * horizonFactor * Utils.planck(lambda, sunTemperature);

        return sun + sky + mieGlow;
    }
